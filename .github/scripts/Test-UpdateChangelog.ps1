...
$TempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("WKOpenVR.PackageChangelog." + [Guid]::NewGuid().ToString('N'))
...
'@
    ...
    Commit-TestChange `
        -Path 'CHANGELOG.md' `
        -Content $InitialChangelog `
        -Subject 'chore: initial changelog'

    Commit-TestChange `
        -Path 'src.txt' `
        -Content "feature`n" `
        -Subject 'feat(package): test changelog append'

    & $Updater -Mode Append -Range 'HEAD~1..HEAD' -RepoRoot $TempRoot
    if ($LASTEXITCODE -ne 0) {
        throw "Update-Changelog Append failed with exit code $LASTEXITCODE"
    }
    ...
    Assert-Contains -Text $Changelog -Expected '**package:** Test changelog append'
    Assert-NotExists -Path (Join-Path $TempRoot 'wiki')
